package com.example.cbqa_0043.ca_guestapp.Utils
...
class Utils {
    ...
    fun editSharedPreferences(activity: AppCompatActivity, map: Map<String, String>): Boolean {
        ...
        val sharedPreferences = activity.getSharedPreferences("mx.com.epcon.pdh", Context.MODE_PRIVATE)
            ...
            for (key in map.keys) {
                if (key.contains("STRING"))
                    ...
                    sharedPreferences.edit().putFloat(
                        key.replace("FLOAT", ""),
                        java.lang.Float.parseFloat(map[key])
                    ).apply()
                else if (key.contains("BOOLEAN"))
                    sharedPreferences.edit().putBoolean(
                        key.replace("BOOLEAN", ""),
                        java.lang.Boolean.parseBoolean(map[key])
                    ).apply()
            }
            return true
        } catch (e: Exception) {
